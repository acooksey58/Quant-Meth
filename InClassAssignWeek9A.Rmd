...
```{r}
("The author uses population sizes as the forcing variable and the outcome variable is the analysis of federal governemnt transfers as the outcome of specific policy thresholds") 

("The weaknees is the average casual effect is only identified at municipalities whose population is 500,000. The external validity the potential applicability of conclusions.")
```
...
```{r}
lm1 <- lm(broad ~ fpm, data = corruption)
lm2 <- lm(broad ~ fpm + pop, data = corruption)
lm3 <- lm(broad ~ fpm + pop + as.factor(pop_cat), data = corruption)
...
("the results are similar for the broad and narrow definitions of corruption. The regressions are positive and provide evidence to support the authors claims")
```
## Question 3
...
plot(all$pop, all$fpm, col = all$col + 2, 
     ...
     xlab = "Population", 
     ylab = "FPM Trasnfer")
abline(v = cut, col = "grey", lty = "dashed")
lines(below.range, below.line, col = "red")
lines(above.range, above.line, col = "blue")
